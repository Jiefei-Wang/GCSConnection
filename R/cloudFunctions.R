...
#' private data, the credentials can be set via `gcs_cloud_auth`
#'
#' @param description character(1). The name of the file that you want
#'     to connect to.  It can be either the file name or a full path
#'     to the file.
#' @param open character(1). A description of how to open the
#'     connection.  See details for possible values. If not specified,
#'     the default value will be "rb" if a credential is set or "rbp"
#'     if not.
...
#'     add a "/" at the end of the path if the path is a folder path.
#'
#' @param recursive logical(1). Whether recursively copy the files in
#'     the subfolders.
#'
...
gcs_cp_internal <- function(from, to, from_cloud, to_cloud, is_folder,
                            recursive) {
    if (is_folder) {
        if (from_cloud) {
        ...
        if (length(subfile_names) != 0) {
            ...
            lapply(seq_along(subfile_names), function(i) {
                gcs_cp_internal(
                    from = from_subfile[i], to = to_subfile[i],
                    from_cloud = from_cloud, to_cloud = to_cloud,
...
#'
#' @param recursive Logical(1), whether recursively query all
#'     subdirectory.  If `TRUE`, all information of the subdirectories
#'     will be downloaded. The time cost can be significantly reduced
#'     if the value is `FALSE`. The parameter only works with
#'     bucket/folder.
#'
...
#'
#' @export
gcs_dir <- function(path, delimiter = TRUE, recursive = FALSE,
                    depth = 2L) {
...
#' @details When the package is loaded, it first searches the
...
#' @export
gcs_cloud_auth <-
    function(json_file, gcloud = FALSE, email = NULL)
{
    ...
    if (gcloud) {
        ...
        if (!is.null(json_file)) {
            tryCatch({
                ...
                package_settings[["credentials"]] <- gar
            },
            warning = function(w) {
                warning(
